...
class Discussion extends CI_Controller {
	...
	{	
        ...
        // $arrayName = array();
        // foreach ($this->input->post('attachment') as $keyvalue) {
        // 	array_push($arrayName, $keyvalue);
        // }
        // print_r($arrayName);
        $skill_required_discussion=implode('|', $this->input->post('attachment'));
...
public function get_emarketpoint(){
 ...
 function discussionfileupload(){
 	 ...
 	 $uploadPath = 'upload/discussion/';
                ...
                $config['allowed_types'] = 'jpg|jpeg|png|doc|docx|pdf|ppt|pptx|xls|xlsx|csv';
                ...
                if($this->upload->do_upload('file')){
                    ...
                    $fileData = $this->upload->data();
                     ...
                     echo   str_replace(' ', '-',$uploadData['file_name']);	
...
public function view_discussion($did){ //view discussion for my created/attendeed 
	...
	$getuser=$this->MainModel->runselect("SELECT *,GROUP_CONCAT(touser_ID)as user FROM trader_notifications WHERE post_discu_ID=".$did." AND type=1");

		if($getuser[0]->user==''){
			$getextuser=$this->session->userdata('userid').',1';
		}else{
			$getextuser=$getuser[0]->user.','.$this->session->userdata('userid').',1';
		}

		$data['getALLuser']=$this->MainModel->runselect("SELECT * FROM trader_user WHERE user_ID NOT IN(".$getextuser.")");
...
$con1 = array('user_ID'=>$this->input->post('intuser'));
$user = $this->MainModel->get_singlerecord('trader_user',$con1);
$con2 = array('user_ID'=>$this->session->userdata('userid'));
$userfrom = $this->MainModel->get_singlerecord('trader_user',$con2);


	$notidata = array(
					'post_discu_ID'=>$this->input->post('did'),
					'fromuser_ID'=>$userfrom->user_ID,
					'touser_ID'=>$user->user_ID,
					'type'=> '1',
					'as_Type'=> $this->input->post('user_Type'),
			...
			$usersubject = "Invitated ".$this->input->post('discussion_title')." Discussion";
			...
			$maildata['mailed_data'] = array(
			                    "username"=>$userfrom->virtual_name,
			                    "tousername"=>$user->virtual_name,
			                    "user_type"=>$this->input->post('user_Type'),
			                    "discussion_id"=>$this->input->post('did'),
			                    'discussion'=>$this->input->post('discussion_title'),
			                    'logo' => base_url().'assets/images/trader-logo.png'
			                );
				$ownercomplete = $this->load->view("templates/invitation",$maildata,true);
				...
				if($user->notification_email != ''){
	      		...
	      		$config = Array(
			        ...
			        redirect("view-discussion/".$this->input->post('did'));
}
