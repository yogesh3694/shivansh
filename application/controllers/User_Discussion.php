...
public function view_defaultdiscussion(){  
    ...
    $showlimit = 1;
    $select = "SELECT tb.join_as,tb.pre_accept,tb.payment_status,des.requirment_detail as description, des.require_presenter as req_pre, des.require_attendee as req_att,des.base_price as price,des.final_price as finalprice, (select count(dscussion_ID) from trader_bid where dscussion_ID = des.discussion_ID and approve_status='1' and join_as='2' and pre_accept='1' AND isActive='1' AND isDelete='0')as presenter,(select count(dscussion_ID) from trader_bid where dscussion_ID = des.discussion_ID and approve_status='1' and join_as='1' and payment_status='1' AND isActive='1' AND isDelete='0')as attendee,tb.bid_ID,(select count(bid_ID) from trader_bid as ctbp where ctbp.dscussion_ID = $did and ctbp.join_as = '2') as totalpre,(select count(bid_ID) from trader_bid as ctbp where ctbp.dscussion_ID = $did and ctbp.join_as = '1') as totalatt, tb.user_ID as biduserid,tb.bid as bidprice,tb.createdDate as biddate,tb.approve_status as status, tu.virtual_name as name, tu.profile_photo as profile, tu.total_skill_points as skillpoints, tu.market_point as markerpoints, CONCAT((select name from trader_countries where country_ID = (SELECT country FROM trader_user WHERE user_ID = tb.user_ID)),', ' ,(select name from trader_cities where city_ID = (SELECT city FROM trader_user WHERE user_ID = tb.user_ID))) as location, (select count(bid_ID) FROM trader_bid as countpr where countpr.join_as = '2' AND countpr.approve_status = '1' AND countpr.pre_accept = '1' AND countpr.user_ID = tb.user_ID ) as presentercount, (select count(bid_ID) FROM trader_bid as countpr where countpr.join_as = '1' AND countpr.approve_status='1' AND countpr.payment_status='1' AND countpr.user_ID = tb.user_ID) as attendeecount,(select COUNT(discussion_ID) FROM trader_discussion WHERE trader_discussion.user_ID =  tb.user_ID and trader_discussion.status = '3' ) as discount FROM `trader_bid` as tb LEFT JOIN trader_user as tu ON tb.user_ID = tu.user_ID left JOIN trader_discussion as des ON tb.dscussion_ID = des.discussion_ID ";
                        ...
                        ?>
                            <div class="trader-listing-detail-tabs-box <?php echo $prcls; ?>">
                                            ...
                                            <div class="modal-content">
                                              ...
                                              <div class="modal-body">
                                                ...
                                                <div class="popup_text">
                                                  <p>You have approved all the presenters for this discussion</p>  
                                                </div>
                                                <?php
                                                } 
                                                else{ ?>
                                                    <div class="popup_text left_text">
                                ...
                                <?php  
                                    if($aspre->status == '1'){
                                        if($aspre->join_as == '2' ){
                                            echo ($aspre->pre_accept == '1' ?'<div class="approve-line"><h3>Approved</h3></div>':'');
                                        }
                                        else
                                        {
                                            echo '<div class="approve-line"><h3>Approved</h3></div>';
                                        }
                                    }
                                 ?> 
                        </div>
                                    ...
                                    <div class="Presenter-detail-inner-detail">
                                        <div class="Presenter-img">
                                            <?php if($asatt->profile!=''){
                                            	...
                                            	$image_path = base_url('upload/profile/' . $asatt->profile);
				                                $thumb_path = preg_replace('~\.(?!.*\.)~', '-96x96.', $image_path);

				                                ini_set('allow_url_fopen', true);

				                                if (getimagesize($thumb_path)) {
				                                    $image_path3 = $thumb_path;
				                                }
                                             ?>
                                        ...
                                        <div class="popup_middile">
                                            <div class="modal-content">
                                              ...
                                              <div class="modal-body">
                                                ...
                                                ?>
                                                    <div class="popup_text">
                                                      <p>You have approved all the attendees for this discussion</p>  
                                                    </div>
                                                <?php
                                                } 
                                                else{ 
                                                    $doller2 = ($descdata->final_price / 100) * $asatt->bidprice;
                                                ?>
                                                    <div class="popup_text left_text">
                                                      <p>Total payable amount for this discussion is <b><?php echo '$'.$asatt->finalprice; ?></b>. User as attendee has bid <?php echo $asatt->bidprice.'% ($'.$doller2.')'; ?> for this discussion. </p>
                                                    </div>
                                                <div class="popup_btn_content">
...
$aspresenter = $this->MainModel->runselect("SELECT tb.join_as,tb.pre_accept,tb.payment_status,tb.bid_ID, tb.user_ID as biduserid,tb.bid,tb.createdDate as biddate,tb.approve_status as status, tu.virtual_name as name, tu.profile_photo as profile, tu.total_skill_points as skillpoints, tu.market_point as markerpoints, CONCAT((select name from trader_countries where country_ID = (SELECT country FROM trader_user WHERE user_ID = tb.user_ID)),', ' ,(select name from trader_cities where city_ID = (SELECT city FROM trader_user WHERE user_ID = tb.user_ID))) as location, (select count(bid_ID) FROM trader_bid as countpr where countpr.join_as = '2' AND countpr.approve_status = '1' AND countpr.pre_accept = '1' AND countpr.user_ID = tb.user_ID) as presentercount, (select count(bid_ID) FROM trader_bid as countpr where countpr.join_as = '1' AND countpr.approve_status='1' AND countpr.payment_status='1' AND countpr.user_ID = tb.user_ID) as attendeecount,(select COUNT(discussion_ID) FROM trader_discussion WHERE trader_discussion.user_ID =  tb.user_ID) as discount FROM `trader_bid` as tb LEFT JOIN trader_user as tu ON tb.user_ID = tu.user_ID left JOIN trader_discussion as des ON tb.dscussion_ID = des.discussion_ID WHERE tb.join_as = '".$_POST['joinas']."' AND tb.dscussion_ID = '".$_POST['discussion']."' AND tb.bid_ID < '".$_POST['bid']."' ORDER BY tb.bid_ID DESC limit $showlimit ");
                    ...
                    <div class="Presenter-detail-inner-detail">
                        <div class="Presenter-img">
                            <?php if($aspre->profile!=''){
                            	...
                            	$image_path = base_url('upload/profile/' . $aspre->profile);
                                $thumb_path = preg_replace('~\.(?!.*\.)~', '-96x96.', $image_path);
                      ...
                      <div class="modal-body">
                        ...
                        if($this->input->post("joinas") == '2'){
                            ...
                            <?php
                                    ...
                                    $doller = ($descdata->final_price / 100) * $aspre->bid;
                               ?>
                                <div class="popup_text left_text">
                                    <p>Total payable amount for this discussion is <b><?php echo '$'.$descdata->final_price; ?></b>. User as attendee has bid <?php echo $aspre->bid.'% ($'.$doller.')'; ?> for this discussion. </p>
                                </div>
                                <div class="popup_btn_content">
                                    <a href="<?php echo base_url() ?>Discussion/approve_bid/<?php echo $aspre->bid_ID.'/'.$aspre->biduserid; ?>" class="btn-bule-outline pop_confirm_btn">confirm</a> 
                                    <button type="button" class="btn-bule-outline" data-dismiss="modal">cancle</button>
